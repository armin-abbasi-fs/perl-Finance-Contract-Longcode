...
sub shortcode_to_parameters {
    ...
    if ($shortcode =~ /^(MULTUP|MULTDOWN)_([\w\d]+)_(-?\d*\.?\d+e[+-]?\d+|\d*\.?\d*)_(\d+)_(\d+)_(\d+)_(\d+(?:m|h|s)?)_(\d+\.?\d*)$/) {
        $bet_type            = $1;
        $underlying_symbol   = $2;
        $stake               = $3;
        ...
        /^([^_]+)_([\w\d]+)_(-?\d*\.?\d+e[+-]?\d+|\d*\.?\d*)_(\d+)(?<start_cond>[F]?)_(\d+)(?<expiry_cond>[FT]?)_(S?-?\d+P?)_(S?-?\d+P?)(?:_(?<extra>[PM])(\d*\.?\d+))?$/)
    {    # Both purchase and expiry date are timestamp (e.g. a 30-min bet)
    ...
    } elsif ($shortcode =~ /^([^_]+)_(R?_?[^_\W]+)_(-?\d*\.?\d+e[+-]?\d+|\d*\.?\d*)_(\d+)_(\d+)t_(\d+)$/) {    # TICKHIGH/TICKLOW contract type with selected tick
        $bet_type          = $1;
        $underlying_symbol = $2;
        $payout            = $3;
        $date_start        = $4;
        $duration          = $5 . 't';
        $selected_tick     = $6;
    } elsif ($shortcode =~ /^([^_]+)_(R?_?[^_\W]+)_(-?\d*\.?\d+e[+-]?\d+|\d*\.?\d*)_(\d+)_(\d+)(?<expiry_cond>[FT]?)$/) {    # Contract without barrier
        $bet_type          = $1;
        $underlying_symbol = $2;
